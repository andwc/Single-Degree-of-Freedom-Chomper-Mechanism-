...
如果不清楚点的对应关系，可将文件最后的注释代码选中（386行开始），ctrl+t一键取消注释
注意：这会大量损失流畅度，造成卡顿。
————————————————————————————————————————————————————————————
仿真时间更改：
    195、196行。
...
在仿真的时候算凸轮角度的时候写了很多if else
    有一些判断是初始条件优化后的结果，不会影响文章中齿数的计算。
    有一些是非完整齿轮的结果。

本文件内含扩展包的函数，使用前需要在主页>附加功能中安装Image Processing Toolbox
...
for i = 0:tick:Time
    ...
    % rectangle('Position', [U2(1)+18/26*24-r_gear,U2(2)-18/26*10-r_gear,2*r_gear,2*r_gear],'Curvature', [1, 1], 'EdgeColor', [0.5,0,0.7], 'LineWidth', 1);
    ...
    % rectangle('Position', [U2(1)+19/26*24-r_gear_dot,U2(2)+19/26*10-r_gear_dot,2*r_gear_dot,2*r_gear_dot],'Curvature', [1, 1], 'EdgeColor', 'b', 'LineWidth', 1);
    ...
    % rectangle('Position', [U2(1)+20.5-r_gear_ddot,U2(2)-r_gear_ddot,2*r_gear_ddot,2*r_gear_ddot],'Curvature', [1, 1], 'EdgeColor', 'g', 'LineWidth', 1);
    ...
    % 点名称
    % text(A2(1), A2(2), 'A', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(B2(1), B2(2), 'B', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(C2(1), C2(2), 'C', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(CC2(1), CC2(2), 'C''', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(D2(1), D2(2), 'D', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(E2(1), E2(2), 'E', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(F2(1), F2(2), 'F', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(G2(1), G2(2), 'G', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(H2(1), H2(2), 'H', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(I2(1), I2(2), 'I', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(J2(1), J2(2), 'J', 'Color', [0.5, 0, 0.7], 'FontWeight', 'bold');
    % text(O_dot2(1), O_dot2(2), 'O_dot', 'Color', 'b', 'FontWeight', 'bold');
    % text(A_dot2(1), A_dot2(2), 'A_dot', 'Color', 'b', 'FontWeight', 'bold');
    % text(C_dot2(1), C_dot2(2), 'C_dot', 'Color', 'b', 'FontWeight', 'bold');
    % text(D_dot2(1), D_dot2(2), 'D_dot', 'Color', 'b', 'FontWeight', 'bold');
    % text(O_ddot2(1), O_ddot2(2), 'O_ddot', 'Color', 'g', 'FontWeight', 'bold');
    % text(A_ddot2(1), A_ddot2(2), 'A_ddot', 'Color', 'g', 'FontWeight', 'bold');
    % text(C_ddot2(1), C_ddot2(2), 'C_ddot', 'Color', 'g', 'FontWeight', 'bold');
    % text(D_ddot2(1), D_ddot2(2), 'D_ddot', 'Color', 'g', 'FontWeight', 'bold');
    % text(U2(1), U2(2), 'U', 'Color', 'k', 'FontWeight', 'bold');
